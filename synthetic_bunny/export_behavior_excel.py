...
def _metric_pass_statuses(row: Dict[str, object], thresholds: Dict[str, float]) -> Dict[str, bool | None]:
    ...
    return {
        ...
        "lift_entropy_vs_graphrag": float(row["lift_entropy_vs_graphrag"])
        ...
        "lift_entropy_vs_random": float(row["lift_entropy_vs_random"])
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-xlsx",
        default="synthetic_bunny/output/behavior_runner/behavior_report.xlsx",
    ...
    for lam in sorted(lambdas):
        ...
        if not lam_rows:
            continue
        lift_entropy_vals = [float(r["lift_entropy_vs_graphrag"]) for r in lam_rows]
        ...
        summary_rows.append(
            {
                "lambda": lam,
                "runs": len(lam_rows),
                "mean_lift_entropy_vs_graphrag": mean(lift_entropy_vals),
                "median_lift_entropy_vs_graphrag": median(lift_entropy_vals),
                "std_lift_entropy_vs_graphrag": pstdev(lift_entropy_vals)
                if len(lift_entropy_vals) > 1
                else 0.0,
                "mean_lift_coverage_vs_graphrag": mean(
                    float(r["lift_coverage_vs_graphrag"]) for r in lam_rows
                ),
