...
def write_combined_html(vector_fig: go.Figure, graph_fig: go.Figure, output_html: Path) -> Tuple[Path, Path]:
    vector_json_path = output_html.with_name(f"{output_html.stem}.vector.json")
    graph_json_path = output_html.with_name(f"{output_html.stem}.graph.json")

    # Keep the HTML light by storing figure payloads in sidecar JSON files.
    vector_json_path.write_text(
        json.dumps(vector_fig.to_plotly_json(), separators=(",", ":")),
        encoding="utf-8",
    )
    graph_json_path.write_text(
        json.dumps(graph_fig.to_plotly_json(), separators=(",", ":")),
        encoding="utf-8",
    )
...
"""
    ...
    return vector_json_path, graph_json_path


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build a Google Sites-friendly interactive 3D embedding bundle "
            "(HTML + sidecar Plotly JSON files)."
        )
    )
    parser.add_argument(
        "--vectors-path",
    ...
    parser.add_argument(
        "--output-html",
        default="presentation/interactive_projection_google_sites.html",
        help="Output HTML path with vector/graph toggle.",
    )
    parser.add_argument(
        "--title",
    ...
    vector_json_path, graph_json_path = write_combined_html(
        vector_fig=vector_fig,
        graph_fig=graph_fig,
