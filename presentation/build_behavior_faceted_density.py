...
def build_lambda_points(
    ...
    per_lambda: Dict[str, Dict[str, np.ndarray]] = {}
    ...
    for lambda_key in lambdas:
        ...
        for method, src in (("graphrag", graphrag_rows), ("random", random_rows), ("bunny", bunny_rows)):
            ...
            for dataset_id in common_ids:
                row = src[dataset_id]
                pts.append((float(row["avg_similarity"]), float(row["avg_entropy"])))
            method_points[method] = np.asarray(pts, dtype=float)
        per_lambda[lambda_key] = method_points
    return lambdas, per_lambda


def compute_axis_limits(per_lambda: Mapping[str, Mapping[str, np.ndarray]]) -> Tuple[Tuple[float, float], Tuple[float, float]]:
    xs: List[float] = []
    ys: List[float] = []
    for method_map in per_lambda.values():
        for pts in method_map.values():
            if pts.size == 0:
                continue
...
def _kde_density_grid(
    ...
    if pts.shape[0] < 3:
        return None
    coords = np.vstack([pts[:, 0], pts[:, 1]])
    try:
        kde = gaussian_kde(coords)
    except Exception:
        x_range = max(float(np.max(pts[:, 0]) - np.min(pts[:, 0])), 1e-6)
        y_range = max(float(np.max(pts[:, 1]) - np.min(pts[:, 1])), 1e-6)
        jitter = np.column_stack(
            [
                rng.normal(0.0, 0.003 * x_range, size=pts.shape[0]),
                rng.normal(0.0, 0.003 * y_range, size=pts.shape[0]),
            ]
        )
        coords = np.vstack([(pts[:, 0] + jitter[:, 0]), (pts[:, 1] + jitter[:, 1])])
        try:
            kde = gaussian_kde(coords)
        except Exception:
            return None
...
def plot_faceted_density(
    run_name: str,
    lambdas: Sequence[str],
    per_lambda: Mapping[str, Mapping[str, np.ndarray]],
    output_path: Path,
    ncols: int,
    grid_points: int,
) -> None:
    ...
    x_grid = np.linspace(x_lim[0], x_lim[1], max(60, grid_points))
    y_grid = np.linspace(y_lim[0], y_lim[1], max(60, grid_points))
    ...
    for idx, lambda_key in enumerate(lambdas):
        ...
        for method in METHOD_ORDER:
            pts = method_map.get(method, np.empty((0, 2), dtype=float))
            if pts.size == 0:
                continue

            z = _kde_density_grid(pts=pts, x_grid=x_grid, y_grid=y_grid, rng=rng)
            if z is None:
                continue
            ...
            if positive.size < 4:
                continue
            level_quantiles = np.quantile(positive, [0.55, 0.72, 0.86, 0.94])
            levels = np.unique(level_quantiles)
            if levels.size < 2:
                continue

            xx, yy = np.meshgrid(x_grid, y_grid)
            ax.contour(xx, yy, z, levels=levels, colors=METHOD_COLORS[method], linewidths=1.15, alpha=0.95)
            ax.scatter(
                pts[:, 0],
                pts[:, 1],
                s=10,
        ...
        ds_count = method_map.get("bunny", np.empty((0, 2))).shape[0]
    ...
    legend_handles = [
        Line2D([0], [0], color=METHOD_COLORS[m], linewidth=2.0, label=m) for m in METHOD_ORDER
    ]
    fig.legend(handles=legend_handles, loc="upper center", ncol=3, frameon=False, bbox_to_anchor=(0.5, 1.02))
    fig.suptitle(f"{run_name} | Faceted Density Contours by Lambda", fontsize=13)
...
def process_run(
    ...
    for ext in formats:
        output_path = out_dir / f"faceted_density_by_lambda.{ext}"
    ...
    for lam in lambdas:
        panel_index.append({"lambda": lam, "dataset_count_common": int(per_lambda.get(lam, {}).get("bunny", np.empty((0, 2))).shape[0])})
    (out_dir / "panel_index.json").write_text(json.dumps(panel_index, indent=2), encoding="utf-8")
    print(f"[OK] {run_name}: wrote faceted density ({', '.join(formats)})")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build faceted density-contour plots by lambda for each behavior run folder.")
    ...
    parser.add_argument("--out-dir-name", default="faceted_density", help="Output subfolder created under each run.")
    parser.add_argument("--ncols", type=int, default=4, help="Number of columns in facet grid.")
    parser.add_argument("--grid-points", type=int, default=120, help="Grid resolution per axis for KDE contours.")
